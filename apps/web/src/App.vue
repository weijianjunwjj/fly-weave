    ...
    <section class="shell-content">
      <header class="topbar">
        ...
        <div class="topbar-actions">
          <span class="connection" :data-status="healthStatus"><i></i>{{ healthStatus === 'connected' ? 'Systems operational' : healthStatus === 'loading' ? 'Connecting' : 'Demo available' }}</span>
...
.shell-content { min-width: 0; flex: 1; margin-left: 224px; }.topbar { position: sticky; z-index: 15; top: 0; display: flex; height: 58px; align-items: center; justify-content: space-between; padding: 0 28px; border-bottom: 1px solid #e1e4e9; background: rgba(255,255,255,.92); backdrop-filter: blur(12px); }.topbar>div { display: flex; align-items: center; gap: 9px; }.breadcrumb { color: #969da9; font-size: 10px; }.topbar i { color: #c0c4cb; font-style: normal; }.topbar strong { color: #434c5c; font-size: 10px; }.topbar-actions { gap: 13px !important; }.connection { display: flex; align-items: center; gap: 6px; color: #717a89; font-size: 9px; }.connection i { width: 6px; height: 6px; border-radius: 50%; background: #d79a2b; }.connection[data-status="connected"] i { background: #23a172; }.topbar-actions button { display: grid; width: 27px; height: 27px; place-items: center; border: 1px solid #e0e3e8; border-radius: 7px; background: #fff; color: #5e6878; cursor: pointer; }.operator-mini { width: 27px; height: 27px; }main { min-height: calc(100vh - 58px); }
@media (max-width: 900px) { .sidebar { width: 70px; }.brand { padding: 0 20px; }.brand>span:last-child,nav p,nav a:not(.active),nav a.active:not([href="#/approvals"]),nav a b,.sidebar-foot>div:not(.operator-avatar),.sidebar-foot>span { display: none; }nav a.active { justify-content: center; padding: 0; }nav a.active span { width: auto; }.shell-content { margin-left: 70px; } }
</style>
